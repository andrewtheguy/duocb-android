#!/usr/bin/env bash
#
# Build, install, launch, and watch the app on a development device.
#
...
#
# By default the Rust core is rebuilt from the sibling ../duocb checkout for the
# device's primary ABI (release profile) and the app links it via
# DUOCB_LOCAL_JNILIBS=1. Then the debug APK is installed, the app launched, and
# logcat tailed for the `duocb` tag (Ctrl-C to stop watching; the app keeps
# running).
#
# Usage:
...
if [ -z "$ADB_SERIAL" ]; then
  attached=()
  while IFS= read -r line; do attached+=("$line"); done < <(adb devices | awk 'NR > 1 && $2 == "device" { print $1 }')
  case "${#attached[@]}" in
...
if [ "$REBUILD_CORE" = 1 ]; then
  ...
  echo "== building libduocb.so for $abi in ../duocb"
  (cd ../duocb && ABIS="$abi" ./build-android.sh release)
fi

if [ "$LOCAL" = 1 ]; then
  export DUOCB_LOCAL_JNILIBS=1
else
  unset DUOCB_LOCAL_JNILIBS
fi
...
echo "== launching"
"${ADB[@]}" shell am start -n com.andrewtheguy.duocb/.MainActivity >/dev/null

if [ "$TAIL_LOG" = 1 ]; then
  echo "== logcat (duocb); Ctrl-C to stop"
  "${ADB[@]}" logcat -c || true
  exec "${ADB[@]}" logcat -v time -s duocb AndroidRuntime:E DEBUG:F
fi
