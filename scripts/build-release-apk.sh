...
# ~/.config/duocb-android/release.jks, override with DUOCB_KEYSTORE). On the
# first run the keystore is created with keytool; the password is taken from
# DUOCB_KEYSTORE_PASSWORD or prompted for, the certificate subject from
# DUOCB_KEY_DNAME (e.g. "CN=duocb, O=andrewtheguy, C=US") or prompted for. Keep
...
# to dist/duocb-android-<versionName>.apk. With --bundle the output is the AAB
# Google Play requires (app/build/outputs/bundle/release/app-release.aab,
# copied to dist/duocb-android-<versionName>.aab), signed with the same
# keystore. The core (libduocb.so) comes from the pinned duocb release unless
# DUOCB_LOCAL_JNILIBS=1 (see README).
#
# Usage:
...
#   DUOCB_KEYSTORE=/path/to/key.jks DUOCB_KEYSTORE_PASSWORD=... scripts/build-release-apk.sh
#
...
if [ "$SIGN" = 1 ]; then
  keystore="${DUOCB_KEYSTORE:-$HOME/.config/duocb-android/release.jks}"
  alias="${DUOCB_KEY_ALIAS:-duocb}"
  password="${DUOCB_KEYSTORE_PASSWORD:-}"
  ...
  if [ ! -f "$keystore" ]; then
    ...
    dname="${DUOCB_KEY_DNAME:-}"
    if [ -z "$dname" ]; then
      read -r -p 'Certificate subject (e.g. "CN=duocb, O=andrewtheguy, C=US"): ' dname
    fi
  ...
  unset DUOCB_KEYSTORE DUOCB_KEYSTORE_PASSWORD
fi
...
version="$(sed -n 's/^duocb\.versionName=//p' gradle.properties)"
mkdir -p dist
if [ "$SIGN" = 1 ]; then
  out="dist/duocb-android-$version.$ext"
else
  out="dist/duocb-android-$version-unsigned.$ext"
fi
cp "$artifact" "$out"
echo "== $out"
